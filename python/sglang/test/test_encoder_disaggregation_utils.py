...
class TestEncoderDisaggregationBase(CustomTestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.encoder_port = f"{int(base_port) + 300}"
        cls.prefill_url = f"http://{cls.base_host}:{cls.prefill_port}"
        cls.decode_url = f"http://{cls.base_host}:{cls.decode_port}"
        cls.encoder_url = f"http://{cls.base_host}:{cls.encoder_port}"
        cls.lb_url = f"http://{cls.base_host}:{cls.lb_port}"
        print(f"{cls.base_host=} {cls.lb_port=} {cls.prefill_port=} {cls.decode_port=} {cls.encoder_port=}")

        cls.process_lb, cls.process_decode, cls.process_prefill, cls.process_encoder = None, None, None, None
    ...
    def tearDownClass(cls):
        for process in [cls.process_lb, cls.process_decode, cls.process_prefill, cls.process_encoder]:
            if process:
                try:
